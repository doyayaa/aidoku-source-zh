...
Aidoku 中文漫画图源集合，每个图源一个独立目录（`sources/{id}/`），便于维护与扩展。

> 需要 **Aidoku 0.7.1+**。图源均为非官方社区图源，内容来自各漫画站点。
...
也可以手动添加：Aidoku → 设置 → 图源列表 → 添加图源列表，填入：

```
https://raw.githubusercontent.com/doyayaa/aidoku-source-zh/gh-pages/index.min.json
```
...
本仓库图源为社区维护的非官方图源，与 Aidoku 及各内容站点无任何关联。若内容站点不希望被收录，请联系移除对应图源。
